...
import math
from scipy.signal import iirfilter, freqz, butter, tf2zpk, dimpulse, dlti, buttord, sosfreqz, sosfiltfilt, sos2zpk, unit_impulse, sosfilt
...
ord, wn = buttord(wp, ws, gpass, gstop, False, fs)
print('Bac cua bo loc la ',ord)
print('Tan so cat ', wn)

# Thiết kế bộ lọc IIR, xuất ra dạng theo dạng ma trận sos
sos = iirfilter(ord, wn, rp=None, rs=None, btype='low', analog=False, ftype='butter', output='sos', fs=fs)
# có thể dùng hàm butter có sẵn của python
b1, a1 = butter(ord, wn  , btype='low' , output='ba', analog=False, fs=fs )
...
plt.xlim(0, fs/2)
plt.ylim(-150, 50)
plt.title('IIR Butterworth filter frequency response')
...
# Vẽ giản đồ cực và zero theo dạng ma trận sos
z, p, k = sos2zpk(sos)
#plt.subplot(2, 2, 1)
...
# Vẽ đáp ứng xung của 100 trọng số đầu tiên
x1= unit_impulse(100)
